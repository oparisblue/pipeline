...
Have you ever wanted to seamlessly perform a group of semi-related actions to a file, or even just raw data?

Lets say your friend sent you a picture of a table of data, with hundreds of visible rows, and you need to quickly sum up all the values in one its columns. Perhaps you would type it out by hand into a calculator, but that's time consuming and error prone. You'd probably end up cropping it to just contain the column you need in one program, running it through an OCR program, copying the output of that tool into a spreadsheet program, and then writing a formula to sum it.
